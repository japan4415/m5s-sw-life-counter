...
// メニュー画面
// ============================================================
// 8 項目を画面中央に縦並べし、選択中の項目を強調する。
// 長押し確認時は外周に円弧プログレスを表示する。
// 画面中心 (234) を基準に上下対称:
//   先頭 y=157 (距離 77)、末尾 y=311 (距離 77)、中央 y=234。
//
// なぜ 22px 間隔か:
//   7 項目時は 24px 間隔 (y=162..306) だったが、8 項目では
//   24px だと末尾が y=330 になり確認メッセージ (y=330) と衝突する。
//   22px にすることで末尾 y=311 となり、確認メッセージとの隙間 19px を確保する。
//
// 円形画面への収まり検証 (半径 165 の内側に全項目が収まること):
//   最も中心から遠い項目 (y=157, y=311) の中心距離 = 77px。
...
//   末尾 (y=311, textSize=2.0, h=16px): 下端 y=319。確認 (y=330) 上端 y=324。隙間 5px。

// メニュー項目の配置
constexpr int32_t kMenuFirstItemY  = 157;  // 最初の項目の中心 y
constexpr int32_t kMenuItemSpacing = 22;   // 項目間の y 間隔
...
constexpr int32_t kAboutVersionY = 240;  // "v0.2.0"
