...
- **boot_app0.bin も不要**である。boot_app0.bin は ESP32（オリジナル）の 2nd stage bootloader 補助ファイルであり、ESP32-S3 のブートローダは 0x0 から直接起動する
- 初回実機検証で「3 点書き込みのみで起動する」ことを確定する（[検証事項](#-11-検証事項) V-1）
- **バージョンの単一情報源は GitHub Releases のタグ（v*）とする**。merged.bin は作らず 3 点個別書き込みのまま（リリース選択 UI がタグ名でバージョンを表示する）
...
- 各リリースに表示する情報: バージョン（タグ名）・公開日・リリースノート・添付 3 点（bootloader / partitions / firmware）のサイズと **SHA-256**
...
- **初回（フルフラッシュ）**: `erase_flash`（全消去）→ 選択リリースの bootloader + partitions + firmware の 3 点書き込み。NVS（ゲーム履歴・設定）も消えるため、実行前に警告を表示する
...
| リリースのバイナリが取得できない | アセット欠落 / リポジトリの公開範囲 | リリース構成（3 点の bin + SHA-256）の確認を促す。Worker ログでエラー原因を調査 |
...
- バイナリ取得エラー: リリース構成（3 点 + SHA-256 の添付）の確認を促す
...
flowchart LR
    ...
    B --> C["3 点の bin + SHA-256 を<br>GitHub Release に添付"]
...
3. **Release 作成**: アセット名は固定規約 **`bootloader.bin` / `partitions.bin` / `firmware.bin`** とし、`sha256sums.txt`（形式: `<hash>  <filename>`、`sha256sum -b` 相当。スペース 2 個区切り）を生成して Release アセットとして添付する（`softprops/action-gh-release` 等を使用）
...
| **M2: フルフラッシュ検証** | 初回フルフラッシュ（全消去 + 3 点）の検証。バージョン照合・エラーフロー・更新フロー（NVS 温存）の実機確認。**前提: 着手前に §6 (a) のリリース CI を構築し、v* タグで Release を 1 件作成しておく**（リリース選択機能の実データ確認のため） | **実機で最初に 1 回、フルフラッシュで起動確認**（boot_app0.bin なしの 3 点書き込みで起動すること、更新で NVS が温存されること、電源ボタン短押しで起動することを確認） |
...
- M1 では書き込み対象を既知の正常な firmware.bin に限定し、M2 で初回ユーザー相当のフロー（全消去 → 3 点書き込み）を検証する
...
| D-4 | バイナリ | **merged.bin は作らない**（3 点個別書き込みのまま）。バージョンの単一情報源は GitHub Releases タグ |
...
| # | 検証内容 | 理由・影響 |
|---|---|---|
| V-1 | **boot_app0.bin なしの 3 点書き込みで起動するか** | ESP32-S3 では不要と判断したが、実機で確定する必要がある（初回検証で確認） |
